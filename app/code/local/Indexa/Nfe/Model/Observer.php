 ...
 */
class Indexa_Nfe_Model_Observer extends Mage_Core_Model_Abstract {
     ...
     * 
     * @param Indexa_Nfe_Model_Observer $observer 
     */
    public function salesOrderSaveNf($observer) {
        /**
         * register new nfe order to process
         */
        $mNfeOrder = Mage::getModel('indexa_nfe/sales_order');
        /* @var $mNfeOrder Indexa_Nfe_Model_Sales_Order */
    ...
    public function salesOrderShipmentSaveAfter($observer) {
        ...
        $mNfeOrderNf = Mage::getModel('indexa_nfe/sales_order_nf');
        /* @var $mNfeOrderNf Indexa_Nfe_Model_Sales_Order_Nf */

        $nfId = Mage::getModel('indexa_nfe/sales_order')->load($mCurrentShipment->getOrder()->getId(), 'order_id')->getId();
        ...
        $aNfData['status'] = Indexa_Nfe_Helper_Data::NFE_SHIPMENT_STATUS_CREATED;
    ...
    public function nfeProcess() {
        ...
        Mage::getModel('indexa_nfe/nfe')->execute();
    }
    ...
    public function updateInfo() {
        ...
        Mage::getModel('indexa_nfe/nfe')->updateInfo();
    }
    ...
    public function saveFiles() {
        Mage::getModel('indexa_nfe/nfe')->saveFiles();
    }
    ...
    public function salesQuoteAddItem( $observer ){
        ...
        $quoteItem->setRealPrice( $mProduct->getPrice() );
        
//        $quoteItem->setNfeQcom( $mProduct->getNfeQcom() );
//        $quoteItem->setNfeVcom( $mProduct->getNfeVcom() );
//        $quoteItem->setNfeEan( $mProduct->getNfeEan() );
...
//        
    ...
    public function salesOrderNfeBlock( $observer ){
        ...
        if( get_class( $observer->getBlock() ) == self::NFE_BLOCK_SALES_ORDER_VIEW_INFO ){            
            $observer->getTransport()->setHtml( $observer->getTransport()->getHtml() 
                                              . $observer->getBlock()->getLayout()->createBlock('indexa_nfe/adminhtml_sales_order_nfe')->toHtml() 
            );
        }
    }
    ...
    public function formatAddress( $observer ){
        ...
        if( $observer->getType()->getRenderer()->getType()->getCode() != 'pdf' ){
            return $this;
        }
